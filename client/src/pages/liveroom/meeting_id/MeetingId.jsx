...
function MeetingId() {
	...
	const participants = useMemo(
		() => [
			{ id: 'cam-host', name: 'Host Camera', role: 'Presenter', status: 'Live' },
			{ id: 'cam-1', name: 'Lebo M.', role: 'Participant', status: 'Online' },
			{ id: 'cam-2', name: 'Ana K.', role: 'Participant', status: 'Muted' },
			{ id: 'cam-3', name: 'David N.', role: 'Participant', status: 'Online' },
			{ id: 'cam-4', name: 'Nia S.', role: 'Participant', status: 'Online' },
		],
		[]
	)
	...
	const onToggleVideoMaximize = () => {
		if (!canMaximizeVideo || isAutoLayout) {
			return
		}
	...
	return (
		<div className="gl-meeting-page">
			...
			<header className="gl-meeting-toolbar" aria-label="Meeting layout controls">
				<div className="gl-meeting-toolbar__menus">
					<details className="gl-toolbar-menu">
						<summary className="gl-toolbar-control" aria-label="Meeting view options">
							<svg viewBox="0 0 24 24" aria-hidden="true" className="gl-toolbar-control__icon">
								<path d="M4 6h7v5H4V6Zm9 0h7v5h-7V6ZM4 13h7v5H4v-5Zm9 2.5 3-2.5 3 2.5-3 2.5-3-2.5Z" fill="currentColor" />
							</svg>
							<span>View options</span>
							<svg viewBox="0 0 24 24" aria-hidden="true" className="gl-toolbar-control__caret">
								<path d="m7 10 5 5 5-5H7Z" fill="currentColor" />
							</svg>
						</summary>

						<div className="gl-toolbar-menu__panel" role="menu" aria-label="View options menu">
							<button type="button" className="gl-toolbar-menu__item" onClick={onToggleAutoLayout}>
								<svg viewBox="0 0 24 24" aria-hidden="true" className="gl-toolbar-menu__icon">
								...
								<span>{isAutoLayout ? 'Auto layout: On' : 'Auto layout: Off'}</span>
							</button>
							...
							<button type="button" className="gl-toolbar-menu__item" onClick={onToggleVideoMaximize} disabled={!canMaximizeVideo || isAutoLayout}>
								...
								<span>{isVideoEffectivelyMaximized ? 'Restore stage' : 'Maximize video'}</span>
							</button>
							...
							<button type="button" className="gl-toolbar-menu__item" onClick={() => setHasScreenShare((prev) => !prev)}>
								...
								<span>{hasScreenShare ? 'Stop share (demo)' : 'Start share (demo)'}</span>
							</button>
						</div>
					</details>

					<details className="gl-toolbar-menu">
						<summary className="gl-toolbar-control" aria-label="Meeting info">
							<svg viewBox="0 0 24 24" aria-hidden="true" className="gl-toolbar-control__icon">
						...
						<div className="gl-toolbar-menu__panel" role="menu" aria-label="Meeting information menu">
							...
							<div className="gl-toolbar-menu__meta">
								...
								<span>{isAutoLayout ? `Auto mode: ${autoLayoutMode}` : `Manual mode: ${layoutMode}`}</span>
							</div>
							...
							<div className="gl-toolbar-menu__meta">
								...
								<span>{isAutoLayout ? autoLayoutReason : 'Manual controls active'}</span>
							</div>
						</div>
					</details>
				</div>

				<div className="gl-meeting-toolbar__media" role="group" aria-label="Media controls">
					<button
						type="button"
						className={`gl-toolbar-control ${isCameraOn ? '' : 'is-muted'}`}
					...
					<button
						type="button"
						className={`gl-toolbar-control ${isMicOn ? '' : 'is-muted'}`}
					...
					<button
						type="button"
						className={`gl-toolbar-control ${isSpeakerOn ? '' : 'is-muted'}`}
					...
					<button
						type="button"
						className="gl-toolbar-control gl-toolbar-control--leave"
			...
			<section className={layoutClassName} aria-label="Meeting room layout">
				...
				<aside
					className="gl-people"
					aria-label={isGridMode ? 'Meeting gallery cards' : 'Meeting participants'}
				>
							...
							return (
								<article key={person.id} className={`gl-person-card ${isPinned ? 'is-pinned' : ''}`}>
									<div>
										...
										<p className="gl-person-card__meta">
